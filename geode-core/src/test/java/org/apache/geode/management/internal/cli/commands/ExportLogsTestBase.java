 ...
 */
public abstract class ExportLogsTestBase extends JUnit4CacheTestCase {
  protected static final int ONE_MINUTE = 60 * 1000;
  protected static final int ONE_HOUR = 60 * 60 * 1000;
  protected static final String LOG_LEVEL = LogWriterImpl.levelToString(LogWriterImpl.INFO_LEVEL);
  protected static final SimpleDateFormat sf = new SimpleDateFormat("yyyy/MM/dd");

  private boolean useHttpOnConnect = Boolean.getBoolean(CliCommandTestBase.USE_HTTP_SYSTEM_PROPERTY);
  ...
  @Before
  public void setUpExportLogsTestBase() throws Exception {
    this.gfshDir = this.temporaryFolder.newFolder("gfsh_files").getCanonicalPath();
  }

  @After
  public final void tearDownExportLogs() throws Exception {
    destroyDefaultSetup();
  }
  ...
  protected Object[] setUpJMXManagerOnVM(int vm, final Properties props, String jsonFile) {
    Object[] result = Host.getHost(0).getVM(vm).invoke("setUpJmxManagerOnVm" + vm, () -> {
      ...
      getCache().getLogger().info("KIRK: inside manager");
