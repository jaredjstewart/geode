...
public final class ClassPathLoader {
  ...
  public void rebuildClassLoaderForDeployedJars() {
    ClassLoader parent = buildExtLibClassLoader();
  ...
  public ClassPathLoader(boolean excludeTCCL) {
    ...
    this.classLoaderForDeployedJars = new URLClassLoader(new URL[] {}, buildExtLibClassLoader());
  }
  ...
  private static ClassLoader buildExtLibClassLoader() {
    try {
      ...
      if (!EXT_LIB_DIR.exists()) {
        logger.info("EXT lib dir does not exist: " + EXT_LIB_DIR);
      ...
      logger.error("SecurityException reading ext lib dir: ", sex);
    }
